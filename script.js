// ==========================================
// CONFIGURACIÓN FRONTEND (V13 - BOTÓN VISIBLE)
// ==========================================
// PEGA AQUÍ LA URL DE TU GOOGLE SCRIPT (La misma que usas en la App Admin)
...
// URL BASE DE TU GITHUB (Para construir los enlaces)
...
async function fetchData() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // AUTO-ABRIR MODAL SI VIENE DE UN ENLACE
...
function renderServices(items, container) {
    ...
    items.forEach(s => {
        ...
        // Generamos el enlace de GitHub para este ítem
            ...
            <div class="tech-card h-100 overflow-hidden p-0 d-flex flex-column" onclick="openProductModal('${s.uuid}')" style="cursor: pointer; border: 1px solid #333; border-radius: 10px; background: #1a1a1a;">
                ...
                <div class="p-3 d-flex flex-column flex-grow-1">
                    ...
                    <div class="d-grid gap-2 mt-auto">
                        <button class="btn btn-outline-info btn-sm rounded-pill fw-bold" onclick="event.stopPropagation(); copyToClipboard('${githubLink}')">
                           <i class="bi bi-link-45deg"></i> COPIAR ENLACE
                        </button>
                        
                        <a href="https://wa.me/573137713430?text=Hola%20A.S.T.,%20me%20interesa%20el%20servicio:%20${encodeURIComponent(s.nombre)}" target="_blank" class="btn btn-primary btn-sm rounded-pill" onclick="event.stopPropagation()">
            ...
            <div class="product-card h-100 d-flex flex-column" onclick="openProductModal('${p.uuid}')" style="background: #111; border: 1px solid #333; border-radius: 8px; overflow: hidden;">
                ...
                <div class="p-3 d-flex flex-column flex-grow-1">
                    ...
                    <div class="mt-auto pt-3 border-top border-secondary d-grid gap-2">
                        <button class="btn btn-outline-light btn-sm rounded-pill w-100" onclick="event.stopPropagation(); copyToClipboard('${githubLink}')">
                            <i class="bi bi-link-45deg"></i> COPIAR ENLACE
                        </button>
                    </div>
                </div>
...
// --- FUNCIÓN MAESTRA: GENERAR LINK DE GITHUB ---
// Debe coincidir EXACTAMENTE con la lógica del Backend (Código.gs)
...
function copyToClipboard(text) {
    navigator.clipboard.writeText(text).then(() => {
        // Feedback visual
        alert(`✅ Enlace copiado al portapapeles:\n\n${text}\n\n(Pégalo en WhatsApp para ver la imagen)`);
    }).catch(err => {
...
function openProductModal(uuid) {
    ...
    // BOTÓN DENTRO DEL MODAL TAMBIÉN
    const githubLink = generateGitHubLink(p.nombre);
    const btnShare = document.getElementById('modal-p-share');
    // Reemplazamos el botón para asegurar que tenga la nueva lógica
    const newBtn = btnShare.cloneNode(true);
    newBtn.innerHTML = '<i class="bi bi-link-45deg me-2"></i> COPIAR ENLACE';
...
function handleError(c1, c2) {
    const err = `<div class="col-12 text-center text-danger"><p>Error de conexión con el servidor.</p></div>`;
